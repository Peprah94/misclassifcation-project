# This script runs the "fixed-intercov" model with all the simulated data
...
for(iCount in 1:16){
  ...
  rep_estimates <- foreach(iter = seq_along(sim), .packages = c("pbapply", "nimble", "MCMCglmm", "coda", "MCMCpack", "boot", "MASS", "parallel","ggmcmc")) %dopar% {
    tryCatch({ run_simulations(sim[[iter]], type = "op_cov_finter", model_selection = TRUE) }, error=function(e){cat("ERROR :",conditionMessage(e), "\n")})

  }

  #save the results
  save(rep_estimates, file=paste0("fixedIntercov/estimateDataFixedIntercov1",iCount,".RData"))
  stopCluster(cl)
}
